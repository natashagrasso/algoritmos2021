class HeapMax(object):
    ...
    def __init__(self): #nos genera un vector vacio con la cantidad de posiciones que le digamos
        self.elementos = []
        self.tamanio = 0
    #ej: h= Heap(15) ahi le decimos la canidad elementos que necesitamos
    ...
    def agregar(self, datos):   #en la 1era pos se agrega el elemento
        self.elementos.append(datos)
        self.flotar(len(self.elementos)-1)  #le pasamos desde que posicion van a empezar a flotar los elementos
        self.tamanio += 1
    

    def flotar(self, indice):
        padre = (indice-1) // 2 #calculamos el padre
        while(indice > 0 and self.elementos[indice] > self.elementos[padre]): #mientras se pueda hacer un intercambi 
            self.elementos[indice], self.elementos[padre] = self.elementos[padre], self.elementos[indice]
            indice = padre
            padre = (indice-1) // 2
    
    def quitar(self):
        #primero hacemos un intercambio entre el primero y el ultimo 
    ...
    #transforma el vector en un monticulo
