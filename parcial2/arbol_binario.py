...
#los 3 barridos son iguales pero lo que cambia es el orden de las llamadas recursivas
    ...
    def busqueda_proximidad3(self, clave):  #usada en el ej parcial
        if(self.info is not None):
            if(self.izq is not None):
                self.izq.busqueda_proximidad3(clave)
    ...
    def busqueda(self, clave): #es parecida al preorden
        pos = None  
        if(self.info is not None):  
            if(self.info == '792'): #si self.info es igual a lo que estamos buscando
                pos = self  #a pos le asignamos self
            elif(clave < self.info and self.izq is not None): #si lo que estamos buscando es menor a lo que esta en el self.info y en el subarbol izq hay un nodo probamos buscar
                pos = self.izq.busqueda(clave)  
            elif(self.der is not None): #chequeamos que haya un nodo a la derecha
    ...
    def contar_Diplodocus(self, dic): #usado para el ej 23

        if(self.info is not None): #chequeamos que no este vacio 
            if (self.datos['nombre']=='Diplodocus'and self.datos['nombre']=='Diplodocus'in dic): #si ese derrotada_por esta en el diccionario significa que está cargado
                dic[self.datos['nombre']=='Diplodocus'] +=1 #entonces le incrementamos la cantidad 
            elif (self.datos['nombre']=='Diplodocus' and self.datos['nombre']=='Diplodocus'in dic): #si no esta, pero es distinto de vacio, lo tenemos que cargar
                 dic[self.datos['nombre']=='Diplodocus'] =1 #entonces lo carga en el diccionario y se lo pone en valor 1

            print(self.info, self._altura, self.datos['nombre']=='Diplodocus')
            if(self.izq is not None):
                self.izq.contar_Diplodocus(dic)
            if(self.der is not None):
                self.der.contar_Diplodocus(dic)
                       

    def barrido_rex(self,clave = None):
        if(self.info is not None):
            if(self.izq is not None):
                self.izq.barrido_rex()  
            if (self.datos['nombre'] == 'T-Rex'):  
                print(self.info)
            if(self.der is not None):
                self.der.barrido_trex()
